...
func waitPlacementDecision(client client.Client, namespace string, placementName string) (*v1beta1.Placement, string, error) {
	...
	for {
		...
		for _, cond := range placement.Status.Conditions {
			if cond.Type == "PlacementSatisfied" && cond.Status == "True" {
				...
				if placementDecisionName != "" {
					ctx.Log.Info("got placementdecision name " + placementDecisionName)
		...
		ctx.Log.Info(fmt.Sprintf("could not get placement decision, retry in %v seconds", interval))
	...
	for {
		...
		for _, cond := range drpc.Status.Conditions {
			if cond.Type == "Available" && cond.Status != "True" {
				ctx.Log.Info("drpc status Available is not True")
				ready = false
				break
			}
			if cond.Type == "PeerReady" && cond.Status != "True" {
				ctx.Log.Info("drpc status PeerReady is not True")
				ready = false
				break
			}
		}
		if ready {
			if drpc.Status.LastGroupSyncTime == nil {
				ctx.Log.Info("drpc status LastGroupSyncTime is nil")
				ready = false
			}
		}
		if ready {
			ctx.Log.Info("drpc status is ready")
			return nil
		}
		if time.Since(startTime) > time.Second*time.Duration(timeout) {
		...
		ctx.Log.Info(fmt.Sprintf("drpc status is not ready yet, retry in %v seconds", interval))
		time.Sleep(time.Second * time.Duration(interval))
	}
}
...
func waitDRPCPhase(client client.Client, namespace string, drpcName string, phase string) error {
	...
	for {
		...
		if currentPhase == phase {
			ctx.Log.Info("drpc phase is " + phase)
			return nil
		}
		if time.Since(startTime) > time.Second*time.Duration(timeout) {
		...
		ctx.Log.Info(fmt.Sprintf("current drpc phase is %s, expecting %s, retry in %v seconds", currentPhase, phase, interval))
		time.Sleep(time.Second * time.Duration(interval))
	}
}
